...
또한 좁은 뷰포트에서뿐만이 아니라 넓은 뷰포트에서도 화면에 꽉 차는 섹션들을 생성하기로 하였습니다. 이는 엄청나게 넓은 화면들에서 텍스트와 문단들이 길고 단일한 하나로 확장되지 않도록 화면의 최대폭을 제한해야한다는 것을 의미합니다. 이 지점을 약 800px로 선택하도록 하겠습니다.
