...
3. 상세한 설명과 비디오
4. 동작을 위한 상품의 이미지
5. 클레임을 지원하기 위한 정보를 가지는 데이터 테이블
...
이는 프로젝트의 나머지 부분에서 사용할 페이지 뼈대의 대략적인 구역들로 쉽게 변환되어 들어갈 수 있습니다.

{% include_code _code/addstructure.html structure %}

## 콘텐츠를 페이지에 추가하기

사이트의 기본 구조는 완료되었습니다. 우리는 필요한 섹션들과 섹션들 내에서 보여줄 콘텐츠 그리고 이들이 전체적인 정보구조 내에서 어디에 위치해야할지를 알고 있으므로 이제 사이트 구축을 시작할 수 있을 것입니다.

{% include modules/remember.liquid title="Note" inline="true" list=page.notes.styling %}

### 헤드라인과 폼 생성하기

헤드라인과 응답 요청 폼은 우리의 페이지에서 중요한 구성요소입니다. 이들은 반드시 사용자들에게 즉시 보여질 수 있어야 합니다.
...
사용자들이 모바일 디바이스에서 빠르고 간단하게 콘텐트에 진입할 수 있도록 하기 위해 시멘틱 타입(Semantic Type)들을 추가할 것입니다. 예를 들어, 전화번호에 진입했을 때 사용자는 바로 다이얼 패드를 볼 것입니다.
