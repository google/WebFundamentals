...
notes:
  hidden:
    - "Como nota aparte, recuerda que `visibility: hidden` no es lo mismo que `display: none`. El primero hace que el elemento sea invisible, pero este seguirá ocupando espacio en el diseño (por ejemplo, se muestra como un cuadro vacío). En cambio, el segundo (display: none) elimina el elemento por completo del árbol de visualización, por lo que deja de formar parte del diseño."
---
{% wrap content%}
