...
layout: article
title: "不添加任何图片"
description: "有时，效果最好的图片其实并不是图片。如果可以，请使用浏览器的原生功能提供相同或类似的效果。"
introduction: "有时，效果最好的图片其实并不是图片。如果可以，请使用浏览器的原生功能提供相同或类似的效果。浏览器可以产生之前可能需要使用图片才能产生的视觉效果。这就意味着，现在的浏览器已不再需要下载单独的图片文件了，而且还能避免产生图片缩放不正常的问题。图标可以使用 unicode 或特殊的图标字体来呈现。"
authors:
  - petelepage
article:
...
请注意，在使用这些方法时需要注意呈现周期，这一点在移动设备上非常重要。如果过度使用，您会失去已获得的所有效果，而且可能会降低网页加载性能。
