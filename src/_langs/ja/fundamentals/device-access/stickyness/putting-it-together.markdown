...
layout: article
title: "すべてを統合"
description: "議論しているすべてを取りまとめて 1 つの大きな形にしましょう。"
introduction: "議論しているすべてを取りまとめて 1 つの大きな形にしましょう。"
article:
...
[Web Starter Kit](//developers.google.com/web/starter-kit) には、
最も一般的なオプションのサポートが含まれており、初期段階では十分です。
しかし、完全な実装を理解するには、
Web Starter Kit の`<head>` の一部をご覧ください。さらに多くの
`<meta>` タグが追加されています。
