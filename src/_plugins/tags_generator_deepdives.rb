...
module Jekyll
  ...
  # Find all tags used in deep dives.

  class TagGenerator < Generator
    priority :low

    def generate(site)
      spotlights = site.data['articles']['deep-dive']
      ...
      tags.each do |tag|
        ...
        site.pages << TagPage.new(site, site.source, File.join('_langs', site.data['curr_lang'], 'showcase', 'deep-dive', 'tags'), tag, tag_spotlights[tag])
      end
  ...
  class TagPage < Page
    ...
    def initialize(site, base, dir, tag, spotlights)
        ...
        self.read_yaml(File.join(base, '_layouts'), 'tag-page-deepdives.liquid')

        title_text = site.data["localized_strings"][@langcode]["deep-dives_tagged"]
        title_text = title_text ? title_text : site.data["localized_strings"]["en"]["deep-dives_tagged"]
        ...
        self.data['tag'] = @tag
        self.data['deep-dives'] = @spotlights
