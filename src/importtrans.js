    ...
    if (file.endsWith(".html")) {
      ...
      if (content.trim().indexOf("---") === 0) {
        ...
        try {
          ...
          error = true;
          result.error++;
          var err = {"file": file, "lang": lang, "error": ex};
      ...
      newFileName = path.join(commander.wfroot, "_" + lang, newFileName);
      ...
      if (fs.existsSync(newFileName) === true) {
        error = true;
        result.warning++;
        var err = {"file": file, "lang": lang, "error": {"message": "File already exists."}};
        result.details.push(err);
        console.log(colors.red("? %s"), file);
