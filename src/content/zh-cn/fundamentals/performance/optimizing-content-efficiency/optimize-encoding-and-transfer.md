...
[GZIP](http://wikipedia.org/wiki/Gzip) 是一种可以应用于任何字节流的通用压缩工具：启动之后，该工具会记住其中一些先前看到的内容并尝试以一种有效的方式查找并替换重复的数据片段 - 好奇的用户可以查阅 [GZIP 的出色的低层次解释] (https://www.youtube.com/watch?v=whGwm0Lky2s&feature=youtu.be&t=14m11s)。但是，实际上，GZIP 对基于文本的内容压缩效果最好，对于较大的文件，通常会达到高达 70-90% 的压缩率，而通过其他替代算法对已压缩过的资产（例如，大多数图片格式）运行 GZIP 则收效甚微，甚至没有任何提高。
