...
description: アクセシビリティを改善するための適切なスタイル設定の使用
...
チェックリスト](https://webaim.org/standards/wcag/checklist) でも、この点が指摘されています。「現在、どのページ要素にキーボードのフォーカスが存在するのかを視覚的に明確にする（たとえば、ページのタブ間を移動する場合、どこにいるのかがわかるようにする）」必要があります。
...
実際、レスポンシブにデザインすると、[WebAIM チェックリストのルール 1.4.4](https://webaim.org/standards/wcag/checklist#sc1.4.4){: .external } で定義された「テキストのサイズは 2 倍にしても読むことができ、機能できるようにする必要がある」という要件を満たすことができます。
...
[WebAIM のガイドライン](https://webaim.org/standards/wcag/){: .external } では、すべてのテキストで AA（最小）コントラスト比 4.5:1 を推奨しています。例外は、非常に大きなテキストです（デフォルトの本文テキストより 120～150% 大きい）。この場合、推奨されるコントラスト比は 3:1 です。以下に、コントラスト比の違いを示します。
...
WebAIM 自体にも、便利な[カラー コントラスト チェッカー](https://webaim.org/resources/contrastchecker/){: .external }があり、色のペアのコントラストを確認するために使用できます。
...
[WebAIM チェックリストのセクション 1.4.1](https://webaim.org/standards/wcag/checklist#sc1.4.1){: .external } には、「色はコンテンツを伝えたり、視覚要素を区別する単独の手段として用いないこと」と記載されています。さらに、特定のコントラスト要件を満たしていない限り、「色だけを使用して周囲のテキストとリンクを区別しないこと」とあります。このチェックリストでは、アクティブなリンクを示すには、下線などのその他の標示を追加することを推奨しています（CSS の `text-decoration` プロパティを使用）。
