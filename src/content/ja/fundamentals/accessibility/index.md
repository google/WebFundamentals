...
このドキュメント セットは、[アクセシビリティに関する Udacity コース](https://www.udacity.com/course/web-accessibility--ud891){: .external }で扱われている内容の一部をテキストベースで説明したものです。ビデオコースを直接文字に起こすのではなく、コースの元の内容をベースに、アクセシビリティの原則と実践方法をより簡潔に説明することを目的としています。
...
### TL;DR {: .hide-from-toc }
- アクセシビリティの意味とウェブ開発への適用方法について説明します。
- アクセスしやすく使いやすいウェブサイトにする方法について説明します。
- 開発への影響を最小限に抑えながら基本的なアクセシビリティを組み込む方法について説明します。
- 利用可能な HTML 機能とその機能を使用したアクセシビリティの改善方法について説明します。
- 洗練されたアクセシビリティの使用感を実現するための高度なアクセシビリティの手法について説明します。



アクセシビリティとそのスコープや影響を理解できると、より優れたウェブ デベロッパーになることができます。
このガイドを通じて、すべてのユーザーにとってアクセスしやすく使いやすいウェブサイトを構築する方法を理解することができます。


「アクセシビリティ」をサポートすることは難しいことではありません。このガイドでは、最小限の労力でアクセシビリティを改善する簡単な方法、HTML に組み込まれているものを使用してよりアクセスしやすい堅牢なインターフェースを作成する方法、高度な手法を利用して洗練されたアクセス性を実現する方法について説明します。
...
もちろん、多くのデベロッパーはアクセシビリティの意味を漠然と理解しているだけで、行政機関との契約、チェックリスト、スクリーン リーダーと関係があるくらいの認識しかありません。また誤解も多く、たとえば、多くのデベロッパーは、アクセシビリティに対応する場合、楽しく魅力的なエクスペリエンスを捨てて、使いづらく外観が悪いけれどもアクセスできるサイトにしなければならないと考えています。
...
アクセシビリティとはどういうものでしょうか。これから何を学ぶのでしょうか。
...
また、アクセシビリティは、狭い意味で「一般的」なユーザーに含まれないユーザーの操作を意味します。このようなユーザーは、想定と異なる手段でアクセスや操作を行います。具体的には、アクセシビリティは、なんらかの身体障がいや機能低下を抱えるユーザーにとって重要な機能ですが、そのような状況は、身体的な問題ではなかったり、一時的なものである場合があることも念頭に置いてください。



たとえば、身体に障がいがあるユーザーを主な対象として議論しがちですが、なんらかの理由でアクセスできないインターフェースを使用するケースについては、すべてのユーザーに関係があります。スマートフォンで PC 用のサイトを思い通りに使えなかったり、「このコンテンツはお住まいの地域では使用できません」というメッセージが表示されたり、使い慣れたメニューがタブレットでは見つからなかったことはありませんか？これらはすべて、アクセシビリティの問題です。

詳しく学習すると、この広義で一般的な意味におけるアクセシビリティの問題に対処することで、ほとんどの場合はすべてのユーザーのエクスペリエンスが向上することがわかるはずです。例を見てみましょう。

![アクセシビリティが適切ではないフォーム](imgs/pooraccess.jpg)

このフォームには、いくつかのアクセシビリティの問題があります。
...
では、アクセシビリティの魔法をかけて、問題が修正されたフォームを見てみましょう。テキストの色を濃くして、ラベルがラベル付けの対象に近くなるようにデザインを変更します。また、ラベルを修正してチェックボックスに関連付けて、ラベルをクリックするとチェックボックスが切り替わるようにします。




![アクセシビリティが改善されたフォーム](imgs/betteraccess.jpg)

どちらを使いますか。「使いやすいバージョン」と答えた場合は、このガイドの大前提を順調に理解しています。多くの場合、少数のユーザーを完全にブロックするものは、多くのユーザーにとっても問題点です。そのため、アクセシビリティの問題を修正することで、すべてのユーザーのエクスペリエンスが向上します。
...
(WCAG) 2.0](https://www.w3.org/TR/WCAG20/){: .external } を参照します。これは、アクセシビリティのエキスパートによって、「アクセシビリティ」の意味を系統的に説明するためにまとめられたガイドラインとベスト プラクティスのセットです。

いくつかの国では、ウェブ アクセシビリティの法的要件でこれらのガイドラインの使用を義務付けています。
...
この情報が手元にあれば、アクセシビリティへの取り組みの方向性を見定めることができます。また、プロジェクトが記載されている基準を満たしている限り、ユーザーが積極的ににコンテンツにアクセスしていることを確信できます。



##  ユーザーの多様性について

アクセシビリティについて学習すると、世界中のさまざまなユーザーと彼らに影響するさまざまなアクセシビリティに関するトピックを理解できるようになります。さらに詳しく説明するために、ここでは、Google の全盲のテクニカル プログラム マネージャーである Victor Tsaran による有益な質問と回答のセッションを紹介します。
...
残念なことに、最新のウェブ技術の多くは、サイトの作成に利用すると副作用が出てしまいます。そうしたサイトでは、盲目のユーザーがウェブへのアクセスに使用するツールがうまく機能しないのです。それでも、アクセシビリティを活用するメリットの方が多くあります。私たちは、障がいについて、大まかに視覚、運動、聴覚、認識の 4 つのカテゴリに分けると検討しやすいことに気づきました。
...
非常に幅広い領域を取り上げました。ここで学習した内容は次のとおりです。
 ...
 - アクセシビリティの説明およびすべての人に関係している理由
 - WCAG および WebAIM アクセシビリティチェックリスト
 - 考慮すべきさまざまなタイプの障がい
