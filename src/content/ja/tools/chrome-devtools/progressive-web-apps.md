...
![Manifest ペイン](images/manifest.png)
...
モバイル自体で操作をテストするには、[リモート デバッグ](/web/tools/chrome-devtools/debug/remote-debugging/remote-debugging)を使用して実際のモバイル端末を DevTools に接続し、（DevTools で）[**Add to homescreen**] ボタンをクリックして、接続したモバイル端末で「add to homescreen」プロンプトをトリガーします。
...
![[Service Workers] ペイン](images/sw.png)
...
- [**Offline**] チェックボックスをオンにすると、DevTools はオフライン モードになります。これは、[**Network**] パネルや、[コマンド メニュー](/web/tools/chrome-devtools/settings#command-menu)の [`Go offline`] オプションから使用できるオフライン モードと同じです。
...
- [**Push**] ボタンを使用すると、ペイロードを含まないプッシュ通知（[通知](/web/fundamentals/push-notifications/how-push-works)とも呼ばれる）がエミュレートされます。
...
![エラーが発生している Service Worker](images/sw-error.png)

## Service Worker のキャッシュ{:#caches}

[**Cache Storage**] ペインには、（Service Worker の）[Cache API](https://developer.mozilla.org/en-US/docs/Web/API/Cache) を使用してキャッシュされているリソースの読み取り専用のリストが表示されます。

![Service Worker のキャッシュ ペイン](images/sw-cache.png)
...
![Service Worker の複数のキャッシュ](images/multiple-caches.png)
