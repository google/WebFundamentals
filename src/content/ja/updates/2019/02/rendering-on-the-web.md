...
サーバーレンダリングは通常、高速の[ファーストペイント](https://developers.google.com/web/fundamentals/performance/user-centric-performance-metrics#first_paint_and_first_contentful_paint) （FP）および[ファーストコンテンツフルペイント](https://developers.google.com/web/fundamentals/performance/user-centric-performance-metrics#first_paint_and_first_contentful_paint) （FCP）を生成します。サーバー上でページロジックを実行してレンダリングすることにより、クライアントに大量のJavaScriptを送信することを回避できるため、高速な[対話型](https://developers.google.com/web/tools/lighthouse/audits/time-to-interactive) （TTI）を実現できます。サーバーレンダリングでは、テキストとリンクをユーザーのブラウザーに送信するだけなので、これは理にかなっています。このアプローチは、デバイスおよびネットワークの状態の広い範囲でうまく機能し、ストリーミングドキュメント解析などの興味深いブラウザ最適化を開きます。
...
サーバーレンダリングでは、ユーザーがサイトを使用する前にCPUにバインドされたJavaScriptが処理されるのを待つことはほとんどありません。 [サードパーティのJS](https://developers.google.com/web/fundamentals/performance/optimizing-content-efficiency/loading-third-party-javascript/)が避けられない場合でも、サーバーレンダリングを使用して独自のファーストパーティの[JSコストを削減](https://medium.com/@addyosmani/the-cost-of-javascript-in-2018-7d8950fbb5d4)すると、残りの「 [予算](https://medium.com/@addyosmani/start-performance-budgeting-dabde04cf6a3) 」を増やすことができます。ただし、このアプローチには1つの主な欠点があります。サーバーでのページの生成には時間がかかり、多くの場合、 [最初のバイトまでの時間](https://en.wikipedia.org/wiki/Time_to_first_byte) （TTFB）が遅くなる可能性があります。
...
Gats](https://www.gatsbyjs.org)by のようなツールは、開発者の作成したアプリケーションが、ビルドステップで生成されたものよりも、より動的に変化しているように感じられる設計となっています[。
Jek](https://jekyllrb.com)yl [や Metalsmi](https://metalsmith.io)th のような他のツールは、それらの静的な性質をふまえ、よりテンプレート駆動のアプローチを提供します。
...
もしあなたが追加の足掛かりを置ければ、サーバーレンダリング [＋ HTMLキャッ](https://freecontent.manning.com/caching-in-react/)シュは、サーバーのレンダリング時間をがっつり減らすことが可能です。
...
もし、最小の作業[でJavaScriptのバジェッ](https://mobile.twitter.com/HenrikJoreteg/status/1039744716210950144)トを抑え、可能な限り少な[いRTT（往復遅延時間](https://en.wikipedia.org/wiki/Round-trip_delay_time)）で価値を提供すれば、純粋なサーバーレンダリングのパフォーマンスへ近づくことができます。[  
HTTP/2 サーバ](https://www.smashingmagazine.com/2017/04/guide-http2-server-push/)ー通知、ま`たは <link rel=preload&`gt; を使うことで、クリティカルなスクリプトとデータをすぐに配信することができます。これにより、パーサーはより早く動きま[す。  ](/web/fundamentals/performance/prpl-pattern/)
...
注意深く実装すると、これはサーバーレンダリング同様、高速のFCP（First Contentful Paint）を達成します。そし[て、(リ)ハイドレーシ](https://docs.electrode.io/guides/general/server-side-data-hydration)ョンと呼ばれるテクニックを用いて、クライアント上で再レンダリングすることにより「ピックアップ」します。
...
Reactで[は、renderToNodeStream](https://reactjs.org/docs/react-dom-server.html#rendertonodestream)() でストリームは非同期になります - 同期である renderToString と比較して - バックプレッシャーが上手く制御されることを意味します。
...
クローラ[は JavaScript を理解でき](https://web.dev/discoverable/how-search-works)ますが、それらはしばしば、どのようにレンダリングするかを認識している価値がある と[いう制約があり](/search/docs/guides/rendering)ます。
クライアントサイドレンダリングは機能しますが、大抵は追加テストや地道な調査が欠かせません。
ごく最近では、クライアントサイドのJavaScriptによってアーキテクチャが大きく左右される場[合、ダイナミックレンダリ](/search/docs/guides/dynamic-rendering)ングもまた考慮する価値のある選択肢になりました。
