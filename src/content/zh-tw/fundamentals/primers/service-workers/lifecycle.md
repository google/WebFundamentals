...
注：我看到很多人添加 `clients.claim()` 作爲樣板文件，但我自己很少這麼做。
...
注意：`skipWaiting()` 意味着新服務工作線程可能會控制使用較舊工作線程加載的頁面。
