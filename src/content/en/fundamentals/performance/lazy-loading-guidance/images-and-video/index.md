...
typical payload of any given page can be significant. Unfortunately, project
stakeholders may be unwilling to cut any media resources from their existing
applications. Fortunately, lazy loading is a solution that lowers initial page
payload _and_ load time, but doesn't skimp on content.

## What is lazy loading?

Lazy loading is technique where loading of non-critical resources is deferred to
the moment they're needed, rather than loading them up front. Where images are
concerned, lazy loading amounts to initiating a fetch for an image just before
it is scrolled into the viewport. If you've used Lighthouse and examined some
opportunities for improvement, you may have seen some guidance in this realm in
the form of the [Offscreen Images
...
You've probably already seen lazy loading in action. It goes something like
this:

- You navigate to a page, and scroll down as you read content.
- At some point, you scroll a placeholder image into the viewport.
- The placeholder image is replaced by the final image.
...
happen, although you could be using that bandwidth for downloading other
resources that are indeed going to be seen by the user). On metered data plans,
however, loading stuff the user never sees could be a waste of their money.
- It wastes processing time, battery, and system memory. After a media resource is
downloaded, the browser must decode it and render its content in the viewport.

When we lazy load resources, we reduce initial page load time, initial page
weight, and system resource usage. all of which are good for performance. In
this guide, we'll cover some techniques and offer guidance for lazy loading
images and video, as well as [a short list of some commonly used
libraries](/web/fundamentals/performance/lazy-loading-guidance/images-and-video/#lazy_loading_libraries).

## Lazy loading images

Lazy loading images is simple in theory, but the details are actually quite
finicky. Plus there are a couple of distinct use cases that can both benefit
from lazy loading. Let's first start with lazy loading inline images in HTML.
...
most compatible, modern browsers offer a more performant and efficient way to do
the work of checking element visibility via [the intersection observer
API](/web/updates/2016/04/intersectionobserver).

Note: Intersection observer is not supported in all browsers. If compatibility
is crucial, be sure to read [the next
...
event handlers. Ideally, you can conditionally polyfill the feature with a
[polyfill.io](https://polyfill.io) bundle.

Intersection observer is easier to use and read than code relying on various
event handlers. This is because developers only need to register an observer to
watch elements rather than writing tedious element visibility detection code.
All that's left to do for the developer is to decide what to do when an element
is visible. Let's assume this basic markup pattern for our lazily loaded `<img>`
elements:
    ...
    let lazyImageObserver = new IntersectionObserver(function(entries, observer) {
      entries.forEach(function(entry) {
        if (entry.isIntersecting) {
          ...
          // We want to set `srcset` first (assuming it exists), because setting
          // `src` first could potentially cause both images to be loaded.
...
requirements may be such that browser compatibility is critical. If polyfilling
is something you would like to avoid, you could fall back to code using
...
to determine whether an element is in the viewport. Assuming the same markup
pattern as before, we can lazy load elements in the absence of intersection
observer like so:
...
throttle execution, by checking the `active` variable, which contains the
processing state. As images are lazy loaded, they're removed from the elements
array. When the elements array is empty, the scroll event handler is removed.
...
- In Chrome, the default for `preload` used to be `auto`, but as of Chrome 64, it
now defaults to `metadata`. Even so, on the desktop version of Chrome, a portion
of the video may be preloaded using the `Content-Range` header. Firefox, Edge
and Internet Explorer 11 behave similarly.
...
Because default behaviors for `preload` are not set in stone, being explicit is
probably your best bet. In cases where someone initiates playback, using
`preload="none"` is the easiest way to defer loading of video on all platforms.
The `preload` attribute isn't the only way to defer the loading of video
content. [_Fast Playback with Video
Preload_](/web/fundamentals/media/fast-playback-with-video-preload) may give you
some insight into controlling video playback in JavaScript.

### For video acting as an animated GIF replacement

While animated GIFs enjoy wide use, they're subpar to video equivalents,
particularly in file size, which can easily balloon into several megabytes.
Videos of similar visual quality tend to be far smaller.
...
serviceable video-as-GIF replacement that works on most platforms. But how to go
...
It should also be said that you can _not_ assume video autoplay policies in all
browsers will accommodate this technique. Autoplaying video is a pain point for
many people. If a browser thwarts your attempt to automatically play video,
accept that such behavior is a feature, not a bug. All in all, this method is a
reasonable replacement for data-intensive animated GIFs if you're flexible
enough to accept the restrictions browsers may impose.

## Lazy loading libraries

If you're not so concerned about the _how_ of lazy loading and prefer to pick a
script and go, there are plenty of options to choose from:

- [yall.js](https://github.com/malchata/yall.js) is a very small and fast
SEO-friendly lazy loading script I wrote which uses intersection observer.
- [lazysizes](https://github.com/aFarkas/lazysizes) is a full-featured lazy loader
for images and iframes. The pattern it uses is similar to the examples shown
here in that it automatically binds to a `lazyload` class on `<img>` elements,
and requires you to specify image URLs in `data-src` and/or `data-srcset`
attributes. It uses intersection observer, and can be extended with [a number of
plugins](https://github.com/aFarkas/lazysizes#available-plugins-in-this-repo) to
do things such as lazy load video.
- [lozad.js](https://github.com/ApoorvSaxena/lozad.js) is a super lightweight
option which, like yall.js, uses intersection observer only.
...
- If you're seeking a React-specific lazy loader, you might consider
...
loading images for those accustomed to developing React apps.

All of these lazy loading libraries are well documented with plenty of examples.
If you're not one to tinker, any of these will meet your needs.

## Native lazy loading

Currently, Chrome is experimenting with a native lazy loading solution for
`<img>` and `<iframe>` elements by way of [the `lazyload`
attribute](https://whatpr.org/html/3752/urls-and-fetching.html#lazy-loading-attributes).
[While this behavior is currently behind a flag and must be opted
into](https://css-tricks.com/a-native-lazy-load-for-the-web-platform/#article-header-id-3),
it is a promising solution that may finally offer a sorely-needed native
solution to the problem. The `lazyload` attribute can be used like so:

```html
<!-- lazyload for <img> -->
<img src="lazy.jpg" lazyload="on">

<!-- lazyload for <iframe> -->
<iframe src="lazy.html" lazyload="on"></iframe>
```

The `lazyload` attribute takes three values:

- `on`, which tells the browser that the page prefers to lazy load the affected
resource.
- `off`, which tells the browser to load the resource immediately.
- `auto`, which is the default, as if you never specified `lazyload` in the first
place.

Because this feature is currently experimental, you can't expect it to be widely
useful. If browsers don't adopt it widely, not all of your users will benefit
from it. Furthermore, if browser vendors decide this experiment is not workable,
it could be removed. For more information on the `lazyload` attribute, check out
[this CSS-Tricks article](https://css-tricks.com/a-native-lazy-load-for-the-web-platform/#article-header-id-3)
written by [Ben Schwarz](https://twitter.com/benschwarz).
...
It may be tempting to lazy load _every_ media resource on the page with
JavaScript, but you don't always need to. For example, anything above the fold
should be considered critical assets, and not lazy loaded.

The main reason for not lazy loading critical media resources is that lazy
loading delays the loading of those resources until after the DOM is interactive
when scripts have finished loading and begin execution. This is fine for images
below the fold, but it would be faster to load critical resources the normal
way.
...
addressing this optimally in every situation. Only you can make that call.

You may also not want to be so strict about the fold line as the threshold for
triggering lazy loading. It may be more ideal for you to establish a buffer zone
some distance below the fold so that images begin loading well before the user
scrolls them into the viewport. The intersection observer API allows you to
specify a `rootMargin` property in an options object when you create a new
`IntersectionObserver` instance. This will give elements a buffer, which will
trigger lazy loading behavior before the element is in the viewport:
...
If the value for `rootMargin` looks the same as those you'd specify for a CSS
`margin` property, that's because it is! In this case, we're broadening the
bottom margin of the observing element (the browser viewport by default) by 256
pixels. That ensures that the callback will run when an image element is within
256 pixels of the bottom of the viewport.

To achieve this same effect using scroll event handling code, adjust your
`getBoundingClientRect` check to include a buffer, which yields the same result.
...
operations that tax system resources and create jank. At a minimum, consider
using a solid color placeholder occupying the same dimensions as the target
image, or techniques such as
...
beware: It's not available in all browsers, and it adds considerable complexity
to lazy loading code. If you want to use it, you'll need to check for it. Below
shows how you might use `Image.decode()` with a fallback:
...
`Image.decode()` may not do much for you, but it can help cut down on jank when
lazy loading large images and inserting them into the DOM.

### SEO

Search engine crawlers don't experience the web the same way people do. In
Googlebot's case, it uses a custom build of Chrome 41 which doesn't scroll down
the page. If you want to ensure your images get indexed by, say Google or Bing's
image search, you can sniff for Googlebot and Bingbot in the
`navigator.userAgent` property and immediately load all images:

```javascript
if (/(google|bing)bot/i.test(navigator.userAgent) {
  [].slice.call(document.querySelectorAll("img.lazy")).forEach(element => {
    // Load elements immediately.
  });
}
```

Some popular lazy loading scripts do some variation of this check. You can
verify that this code works in both [Bing Webmaster
Tools](https://www.bing.com/webmaster/) and [Google Search
Console](https://search.google.com/search-console/welcome).

### When stuff doesn't load

Sometimes resources will fail to load for one reason or another and errors
occur. When might this happen? Network connectivity is the most common culprit,
but here's a hypothetical scenario: A user has left your site open in a stale
tab for several hours, and comes back to read your content. In the meantime, a
redeployment has occurred. During this deployment, an image resource's name
changes due to hash-based versioning, or is removed altogether. By the time the
user lazy loads the image, the now-unavailable resource fails to load.

While this is admittedly a rare occurrence, it may behoove you to have a backup
plan if lazy loading fails for _any_ reason. For images, such a solution may
...
What you decide to do in the event of an error depends on your application. You
could replace the image placeholder area with a button that allows a person to
attempt to load the image again, or display a helpful error message. Whatever
you decide to do, it's never a bad idea to signal to someone when an error has
occurred.
...
Then we put a very small inline script in the `<head>` before any CSS which
removes the `no-js` class from the `<html>`:

```html
<script>document.documentElement.classList.remove("no-js")</script>
```

Finally, we sprinkle on some CSS to hide elements with a class of `lazy` when
