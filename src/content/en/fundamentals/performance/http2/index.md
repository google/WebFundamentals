...
transported between the client and server, both of whom manage the entire
...
on-web-performance/#latency-as-a-performance-bottleneck).
...
Firefox, and Opera, and a rapidly growing number of sites, both large (e.g.,
...
Observing the above trend, the HTTP Working Group (HTTP-WG) kicked off a new
...
Over the next few years SPDY and HTTP/2 would continue to coevolve in parallel,
...
First versions of the HTTP protocol were intentionally designed for simplicity
...
HTTP](https://hpbn.co/brief-history-of-http/). As such, HTTP/0.9-1.x delivered
exactly what it set out to do: HTTP is one of the most ubiquitous and widely
adopted application protocols on the Internet.
...
unaffected, but the way they are encoded while in transit is what’s different.
...
particular stream, and all of which are multiplexed within a single TCP
...
prefer to receive the responses. In turn, the server can use this information to
...
dependent on the "root stream". Declaring a stream dependency indicates that, if
possible, the parent stream should be allocated resources ahead of its
dependencies—e.g., please process and deliver response D before response C.
...
1. Neither stream A nor B specify a parent dependency and are said to be dependent
...
2. D is dependent on the root stream; C is dependent on D. Thus, D should receive
   full allocation of resources ahead of C. The weights are inconsequential because 
   C’s dependency communicates a stronger preference.
3. D should receive full allocation of resources ahead of C; C should receive full 
   allocation of resources ahead of A and B; stream B should receive one-third of 
   the resources allocated to stream A.
4. D should receive full allocation of resources ahead of E and C; E and C should 
   receive equal allocation ahead of A and B; A and B should receive proportional 
...
further optimizations in the browser—e.g., we can change dependencies and
...
stream in particular order using stream prioritization. While this may seem
...
(i.e., client, intermediaries, and origin servers), which reduces the overall
operational costs and improves network utilization and capacity. As a result,
the move to HTTP/2 should not only reduce the network latency, but also help
...
control window down to zero, and then resume it later—e.g., fetch a preview or
first scan of an image, display it and allow other high priority fetches to
proceed, and resume the fetch once more critical resources have finished
loading.
...
the same results, but with additional performance benefits:
...
stream (via a RST_STREAM frame) if it wants to (e.g., the resource is already in
cache), which is an important improvement over HTTP/1.x. By contrast, the use of
resource inlining, which is a popular "optimization" for HTTP/1.x, is equivalent
to a "forced push": the client cannot opt-out, cancel it, or process the inlined
...
opened; disable server push entirely. These preferences are communicated via the
SETTINGS frames at the beginning of the HTTP/2 connection and may be updated at
any time.
...
2. It requires that both the client and server maintain and update an indexed 
   list of previously seen header fields (i.e., establishes a shared compression 
   context), which is then used as a reference to efficiently encode previously 
   transmitted values.
...
Note: The definitions of the request and response header fields in HTTP/2 remain
...
compress all HTTP headers, which delivered 85%–88% reduction in the size of the
transferred header data, and a significant improvement in page load time
latency:
