...
notes:
  ...
  permutations: "Be sure to include all permutations of file names, for example our app is served from <code>index.html</code>, but it may also be requested as <code>/</code> since the server sends <code>index.html</code> when a root folder is requested. You could deal with this in the <code>fetch</code> method, but it will require special casing which may become complex."
...
work when online, or offline, or even if the user has an intermittent, slow 
connection. To achieve this, we need to cache our App Shell using Service 
Workers, so that it's always available quickly and reliably. 
</p>
...
about what they can do, the life cycle and more.
...
First, create a blank file called `service-worker.js` in your app root folder. 
This `service-worker.js` file must live in the application root because the scope 
for the service workers is defined by the directory where the file exists. 
...
When it's fired, it should open the caches object and populate it with the 
assets necessary to load the app shell. Add this code to your `service-worker.js` 
file:
...
Unfortunately `cache.addAdd` is atomic, if any of the files fail, the entire cache 
step will also fail!
...
The next step is to serve the App Shell from the local cache.
...
Stepping from inside, out, `caches.match` evaluates the web request that triggered 
the `fetch` event, and checks to see if it's available in the cache. It then 
...
### Cache depends on the cache key being updated for every change

For example this caching method is that the unless the cache key is changed 
every time the content is changed, the cache will not be updated, and the old 
content will be served. So be sure to remember to change the cache key with 
every change as you're working on your project!
...
* Once a service worker has been unregistered, it may remain listed and until 
  the browser window that it was in is closed.
* If multiple windows are open to your app are open, the new Service Worker will 
