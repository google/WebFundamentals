...
techniques can substantially improve the user experience. In this article,
we’ll summarize some ideas around offline data storage* *for PWAs — think JSON
...
machinery (e.g. transactions, schema versioning) that comes the IndexedDB library.
...
10’s IndexedDB and PouchDB have found it to be a little slow. Until more research has
been done here, your mileage may vary. Please test and file browser bugs so the folks
@webkit and related OSS library authors can take a look. LocalForage, PouchDB, YDN and
Lovefield use WebSQL in Safari by default due to UA sniffing (there wasn’t an
efficient way to feature-test for broken IndexedDB at the time). This means these
libraries will work in Safari 10 without extra effort (just not using IndexedDB
directly).)
...
[tracks](https://pouchdb.com/faq.html#data_limits) IndexedDB storage behavior.
...
application. It also enables requesting more storage if needed.  A newer
...
![Promise-ized IndexedDB](images/image_1.jpg)
