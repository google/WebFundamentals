...
You probably noticed that the call to `subscribe()` contains a parameter called `applicationServerKey`. It's generated by your server and should already be present in the app when the user downloads it. We were saving all server side issue for the next section. Since the web app needs this value to subscribe a user to push messages, we need to cover it now.  
...
+  Your app should pass the key back to the server as an array of eight-bit unsigned integers.
