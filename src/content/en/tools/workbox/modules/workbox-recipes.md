...
The offline fallback will only be applied if there's a matching [route](/web/tools/workbox/modules/workbox-routing) for a given request. When using offline fallback alongside other recipes, like for the page and image caches, a route will automatically be set up for those requests. When using offline fallback on its own, you will need to explicitly create a route that matches page and image requests. The simplest approach is to use the [`setDefaultHandler()`](/web/tools/workbox/modules/workbox-routing#set_a_default_handler) method to create a route that applies the [`NetworkOnly`](/web/tools/workbox/modules/workbox-strategies#network_only) strategy to all requests, as shown below.
