...
Note: As always – **this is not production-ready code**. The code makes a couple assumptions to make
the code (and the video) easier to follow at the cost of generality. Our main goal is to convey
concepts and demystify buzzwords.
...
* **Code splitting**: Code splitting is the process of *not* shipping one big bundle with all of
your JavaScript to the client. Instead, the code is split into multiple bundles. One specific way of
splitting your code is to use route-based chunking.
* **Route-based chunking**: Route-based chunking creates bundles that are related your app’s routes.
By analyzing your routes and their dependencies, we can make decision what modules go into which
bundle.
...
With natives ES6 modules, every JavaScript module can import its own dependencies. When the browser
...
their own. The danger is that the browser ends up with a cascade of fetches that lasts for multiple
round-trip times (RTT) before the code can finally be executed.

**ADD IMAGE for loose modules**

#### Bundling

Inlining all your modules into one single bundle (“bundling”) will make sure the browser has all the
code it needs after 1 RTT and can start running the code more quickly. This, however, forces the
user to download a lot of code that is not needed, so bandwidth and time has been wasted.
...
Code splitting is the middle ground. We are willing to invest additional RTTs to get network
efficiency by only downloading what we need and better caching efficiency by making the number of
modules per bundle much smaller. If the bundling is done right, the total number of RTTs will be
much lower than with loose modules. Finally, we could make use of [pushing
mechanisms](/web/fundamentals/performance/prpl-pattern/) like `link[rel=preload]` or HTTP/2 Push to
save additional RTTs if needed.
...
efficient not very babel idiomatic. For a more in-depth guide to authoring Babel plugins, I
...
JavaScript module. You might be tempted to resort to regular expressions, but be aware that regular
...
A plugin can provide a `visitor` object. The visitor contains a functions for any node type that the
plugin wants to handle. If a node of that type is encountered while traversing the AST, the
corresponding method of the `visitor` object will be called and get that node as a parameter. In the
example above, the `ImportDeclaration` method would be called for every `import` declaration in the
...
To build the dependency tree of a module, we will parse that module file and create a list of all
...
dependencies by looking the nodes that appear in every tree. To determine the set of shared
dependencies, we will flatten and deduplicate our forest and filter to only keep the elements that
appear in all trees.
...
To bundle our set of shared dependencies, we could just concatenate all the files contained therein.
Two problems arise when using that approach: The first problem is that the bundle will still contain
import statements which will make the browser attempt to fetch resources. The second problem is that
the dependencies’ dependencies are not being bundled.

So we have to be a little smarter than just using file concatenation. In this case, we are going to
write _another_ babel plugin.

Note: I am convinced someone with more experience with Babel and their APIs would be able to do all
of this in one pass. For the sake of brevity and clarity, I chose to write multiple plugins and
parse the same file multiple times so that the steps are truly separate.

The code is fairly similar to our first plugin, but instead of just extracting the imports, we will
also be replacing them with a bundled version of the imported file:
...
and demystify_ code splitting. The result works – but it’s rather specific to our demo site and will
