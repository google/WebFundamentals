...
Synchronous copy & paste might seem fine for small bits of text, but there are a
...
might a dedicated clipboard API look like if we wanted to address blocking and
permissions problems?

That's the new [Async Clipboard API], the text-focused portion of which we're
shipping in Chrome 66.  It's a replacement for execCommand-based copy & paste
...
Text can be copied to the clipboard by calling `writeText()`.  Since this API is
asynchronous, the `writeText()` function returns a Promise that will be resolved
or rejected depending on whether the text we passed is copied successfully:
...
Similarly, we can write this as an [async function], then await the return of
`writeText()`:
...
Much like copy, text can be read from the clipboard by calling `readText()` and
...
Prior to the introduction of the Async Clipboard API, we had a mix of different
copy & paste implementations across web browsers.
...
to be copied is a string not present in the DOM, we have to inject and select
it:
