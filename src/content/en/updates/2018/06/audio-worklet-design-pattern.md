...
[this post](/web/updates/2017/12/audio-worklet).
  ...
  - [AudioWorklet](https://webaudio.github.io/web-audio-api/#audioworklet) (AW):
    A special script file loader for the AudioWorklet operation. Belongs to
    BAC. A BAC can have one AW. The loaded script file is evaluated in the
    AudioWorkletGlobalScope and is used to create the AudioWorkletProcessor
    instances.
  - [AudioWorkletGlobalScope](https://webaudio.github.io/web-audio-api/#audioworkletglobalscope)
    (AWGS): A special JS global scope for the AudioWorklet operation. Runs on a
    dedicated rendering thread for the WebAudio. A BAC can have one AWGS.
  - [AudioWorkletNode](https://webaudio.github.io/web-audio-api/#audioworkletnode)
    (AWN): An AudioNode designed for the AudioWorklet operation. Instantiated
    from a BAC. A BAC can have multiple AWN similarly to the native AudioNodes.
  - [AudioWorkletProcessor](https://webaudio.github.io/web-audio-api/#audioworkletprocessor)
    (AWP): A counterpart of the `AudioWorkletNode`. The actual guts of the
    AudioWorkletNode processing the audio stream by the user-supplied code. It
    is instantiated in the AWGS when a AWN is constructed. An AWN can have one
    matching AWP.
...
instantiation:
  ...
  1. Instantiate a WASM module by loading the glue code into the
  AudioWorkletGlobalScope (AWGS) via `audioContext.audioWorklet.addModule()`.
  1. Instantiate a WASM module in the main scope, then transfer the module via
  the AWN's constructor options.

The decision largely depends on your design and preference, but the idea is that
the WASM module can generate a WASM instance in the AWGS, which becomes an audio
processing kernel within an AWP instance.
...
<figure>
  ...
  <figcaption>
    WASM module instantiation pattern A: Using <code>.addModule()</code> call
  </figcaption>
</figure>
...
These options ensure the synchronous compilation of a WASM module in the AWGS.
It also appends the AWP's class definition in `mycode.js` so it can be loaded
after the module is initialized. The primary reason to use the synchronous
compilation is that the promise resolution of `audioWorklet.addModule()` does
...
<figure>
  ...
  <figcaption>
    WASM module instantiation pattern B: Using AWN constructor's cross-thread
    transfer
  </figcaption>
</figure>
...
dynamically after the AWGS starts rendering the audio stream. Depending on the
...
constructor can definitely be useful.
...
An AudioWorkletNode (AWN) and AudioWorkletProcessor (AWP) pair is designed to
work like a regular AudioNode; AWN handles the interaction with other codes
while AWP takes care of internal audio processing. Because a regular AudioNode
processes 128
...
at a time, AWP must do the same to become a first-class citizen. This is one of
the advantages of the AudioWorklet design that ensures no additional latency due
to internal buffering is introduced within the AWP, but it can be a problem if a
processing function requires a buffer size different than 128 frames. The common
solution for such case is to use a ring buffer, also known as a circular buffer
or a FIFO.

Here's a diagram of AWP using two ring buffers inside to accommodate a WASM
function that takes 512 frames in and out. (The number 512 here is arbitrarily
picked.)
...
The algorithm for diagram would be:
  ...
  1. AWP pushes 128 frames into the **Input RingBuffer** from its Input.
  ...
  1. AWP pulls 128 frames from the **Output RingBuffer** to fill its **Output**.
...
This pattern is useful when replacing ScriptProcessorNode (SPN) with AWN. Since
SPN allows the developer to pick a buffer size between 256 and 4096 frames, so
the drop-in substitution of SPN with AWN can be difficult and using a ring
buffer provides a nice workaround. A audio recorder would be a great example
that can be built on top of this design.
...
Note: SharedArrayBuffer (SAB) is disabled by default at the time of writing. Go
to `chrome://flags` and enable _SharedArrayBuffer_ to play with this feature.
...
(DWGS) solely for audio processing. In Chrome, WorkerGlobalScope runs on a lower
...
(AWGS). DWGS is less constrained in terms of the API surface available in the
scope. Also you can expect better support from Emscripten because the Worker API
has existed for some years.

SharedArrayBuffer plays a critical role for this design to work efficiently.
Although both Worker and AWP are equipped with asynchronous messaging
...
by AWP and process the audio when it wakes up. Along with SharedArrayBuffer
(SAB), Atomics API makes this mechanism possible.

Note that the synchronization of two threads are rather loose. The onset of
`Worker.process()` will be triggered by `AWP.process()` method, but the AWP does
not wait until the `Worker.process()` is finished. This is by design; the AWP is
driven by the audio callback so it must not be synchronously blocked. In the
worst case scenario the audio stream might suffer from duplicate or drop out but
it will eventually recover when the rendering performance is stabilized.

#### Setting Up and Running

As shown in the diagram above, this design has several components to arrange:
WGS, AWGS, SABs and the main thread. The following steps describe what should
happen in the initialization phase.

##### Initialization

  1. [Main] AWN constructor gets called.
    1. Create Worker.
    1. The associated AWP will be created.
  1. [DWGS] Worker creates 2 SABs. (one for shared states and the other for
    audio data)
  1. [DWGS] Worker sends SAB references to AWN.
  1. [Main] AWN sends SAB references to AWP.
  1. [AWGS] AWP notifies AWN that the setup is completed.

Once the initialization is completed, `AWP.process()` starts to get called. The
following is what should happen in each iteration of the rendering loop.
...
<figure>
  ...
  1. [AWGS] `AWP.process(inputs, outputs)` gets called for every render quantum.
  ...
  1. [DWGS] Worker waits (sleeps) for the wake signal from `AWP.process()`.
    When it wakes up:
...
Being able to run WebAssembly on AWGS unlocks huge potential for
high-performance audio processing on the web. For large-scale audio applications
