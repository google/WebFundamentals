...
सीएसएस पेंट एपीआई आपको जब भी एक सीएसएस संपत्ति किसी छवि की अपेक्षा करता है तो प्रोग्रामेटिक रूप से एक छवि उत्पन्न करने की अनुमति देता है। `background-image` या `border-image` जैसे गुण आमतौर पर `url()` के साथ छवि फ़ाइल लोड करने के लिए या सीएसएस अंतर्निहित कार्यों जैसे `linear-gradient()` के साथ उपयोग किए जाते हैं। उन लोगों का उपयोग करने के बजाय, अब आप _paint worklet_ को संदर्भित करने के लिए `paint(myPainter)` का उपयोग कर सकते हैं।

### एक पेंट वर्कलेट लिखना

`myPainter` नामक एक पेंट वर्कलेट को परिभाषित करने के लिए, हमें `CSS.paintWorklet.addModule('my-paint-worklet.js')` का उपयोग करके एक सीएसएस पेंट वर्कलेट फ़ाइल लोड करने की आवश्यकता है। उस फ़ाइल में हम एक पेंट वर्कलेट क्लास पंजीकृत करने के लिए `registerPaint` फ़ंक्शन का उपयोग कर सकते हैं:
...
`paint()` कॉलबैक के अंदर, हम `ctx` का उपयोग उसी तरह कर सकते हैं जैसे हम `CanvasRenderingContext2D` करेंगे क्योंकि हम इसे `<canvas>` से जानते हैं। यदि आप जानते हैं कि `<canvas>` में कैसे आकर्षित किया जाए, तो आप एक पेंट वर्कलेट में आकर्षित कर सकते हैं! `geometry` हमें हमारे निपटारे में कैनवास की चौड़ाई और ऊंचाई बताता है। `properties` मैं इस लेख में बाद में समझाऊंगा।

नोट: एक पेंट वर्कलेट का संदर्भ `<canvas>` संदर्भ के समान 100% नहीं है। अभी तक, पाठ प्रतिपादन विधियां गायब हैं और सुरक्षा कारणों से आप कैनवास से पिक्सेल वापस नहीं पढ़ सकते हैं।
...
नोट: लगभग सभी नए एपीआई के साथ, सीएसएस पेंट एपीआई केवल HTTPS (या `localhost`) पर उपलब्ध है।
...
सौभाग्य से, पेंट वर्कलेट अन्य सीएसएस गुणों तक पहुंच सकता है, जहां अतिरिक्त पैरामीटर `properties` खेल में आता है। कक्षा को एक स्थिर `inputProperties` विशेषता देकर, आप कस्टम गुणों सहित किसी भी सीएसएस संपत्ति में परिवर्तनों की सदस्यता ले सकते हैं। मान आपको `properties` पैरामीटर के माध्यम से दिए जाएंगे।
...
नोट: रंगों को पैरामीटर करना भी बहुत अच्छा होगा, है ना? Spec तर्कों की एक सूची लेने के लिए `paint()` फ़ंक्शन के लिए अनुमति देता है। यह सुविधा अभी तक क्रोम में लागू नहीं की गई है, क्योंकि यह भारी रूप से हुडिनी के गुणों और मूल्यों एपीआई पर निर्भर करती है, जिसे अभी भी जहाज से पहले कुछ काम की आवश्यकता है।
...
सीएसएस पक्ष के लिए, आपके पास दो विकल्प हैं। आप `@supports` का उपयोग कर सकते हैं:
...
पेंट वर्कलेट के लिए ब्राउज़र _with_ समर्थन में, `background-image` की दूसरी घोषणा पहले को ओवरराइट कर देगी। पेंट वर्कलेट के लिए ब्राउज़र _without_ समर्थन में, दूसरी घोषणा अमान्य है और इसे पहले घोषणापत्र को प्रभावी रूप से छोड़कर त्याग दिया जाएगा।
...
## "बॉक्स" के बाहर सोचकर ज्यादातर लोग पृष्ठभूमि छवियों और सीमा छवियों के बारे में सोचने लगते हैं जब वे पेंट वर्कलेट के बारे में सीखते हैं। पेंट वर्कलेट के लिए एक कम सहज उपयोग केस `mask-image` है जो डोम तत्वों को मनमाने ढंग से आकार देता है। उदाहरण के लिए एक [हीरा](https://googlechromelabs.github.io/houdini-samples/paint-worklet/diamond-shape/):
...
नोट: ब्रेकपॉइंट्स वर्तमान में सीएसएस पेंट एपीआई में समर्थित नहीं हैं, लेकिन बाद में क्रोम के रिलीज में सक्षम हो जाएंगे।
