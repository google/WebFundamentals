...
서비스워커 사용시 설치 과정에서 이미지를 캐쉬하려면 아래와 같은 옵션이 존재합니다:

1. `<picture>` 요소와 `srcset` 속성이 요청할 모든 이미지를 설치
2. single low-res version of the image 설치
3. single high-res version of the image 설치

모든 이미지를 다운로드 하면 저장공간이 낭비되므로 현실적인 옵션 2 나 3을 골라야 합니다.

설치 시에 low res version 를 사용한다고 하면 페이지가 로딩될 때 높은 res 이미지들을 받아와야 합니다.
하지만 만약 높은 해상도 이미지가 실패한 경우, 낮은 해상도로 시도하게 됩니다.
이 방법이 괜찮아 보이더라도 한가지 문제점이 있습니다.
...
여기서 브라우저는 2x 화면에 맞는 이미지를 예상할 것이고, 결국 이미지는 400x400 CSS 픽셀 대신에 200x200 CSS 픽셀로 표현됩니다.
이를 해결하는 방법은 이미지의 높이와 너비를 고정하는 것입니다.
...
`<picture>` 요소의 경우에 너비와 폭 고정이 이미지가 어떻게 생성되고 사용하는 것에 달려 있기 때문에 상당히 어렵습니다.
