...
| 속성   |      표시 시기                                                       |
|----------|:-----------------------------------------------------------------|
| Aggregated time | [중첩된 이벤트](/web/tools/chrome-devtools/profile/evaluate-performance/timeline-tool#view-nested-events)가 있는 이벤트의 경우, 각 범주의 이벤트에서 소요된 시간.|
| Call Stack | [하위 이벤트](/web/tools/chrome-devtools/profile/evaluate-performance/timeline-tool#view-nested-events)가 있는 이벤트의 경우, 각 범주의 이벤트에서 소요된 시간.|
| CPU time | 기록된 이벤트에서 소요된 CPU 시간.|
| Details | 이벤트에 대한 기타 세부정보.|
| Duration(타임스탬프에서) | 하위 항목을 모두 포함하여 이벤트가 완료되는 데 걸린 시간. 타임스탬프는 이벤트가 발생한 시간으로, 기록이 시작된 시간에 상대적입니다.|
| Self time    | 하위 항목 없이 이벤트에서 걸린 시간.|
...
| 속성 | 설명 |
|-------|:----------|
|Resource|요청한 리소스의 URL.|
|Preview|요청한 리소스의 미리보기(이미지만 해당).|
|Request Method|요청에 사용된 HTTP 방식(예: GET 또는 POST).|
|Status Code|HTTP 응답 코드.|
|MIME Type|요청한 리소스의 MIME 유형입니다.|
|Encoded Data Length|바이트 단위의 요청한 리소스의 길이.
...
| 이벤트 | 설명 |
|-------|:----------|
|애니메이션 프레임 작동| 일정이 예약된 애니메이션 프레임이 작동하여 그 콜백 핸들러를 호출했습니다.|
|애니메이션 프레임 취소| 일정이 예약된 애니메이션 프레임이 취소되었습니다.|
|GC 이벤트| 가비지 수집이 발생했습니다.|
...
|Event| JavaScript 이벤트(예; "mousedown" 또는 "key").|
|Function Call| 최상위 단계 JavaScript 함수 호출이 있었습니다(브라우저가 자바스크립트 엔진에 진입하는 경우에만 나타남).|
...
|Time| [console.time()](/web/tools/chrome-devtools/debug/console/console-reference#consoletimelabel)으로 불리는 스크립트.|
|Time End| [console.timeEnd()](/web/tools/chrome-devtools/debug/console/console-reference#consoletimeendlabel)로 불리는 스크립트.|
...
|-------|:----------|
|Timer ID|타이머 ID입니다.
|시간 초과|타이머에 지정된 제한 시간.|
|Repeats|타이머가 반복되는지 지정하는 부울 값.|
|Function Call|호출된 함수.|
...
|Nodes that need layout|Layout 기록의 경우, 재레이아웃이 시작되기 전에 레이아웃이 필요한 것으로 표시된 노드 수. 이들은 대개 개발자 코드로 무효화된 노드이며, 재레이아웃 루트로 향하는 상향 경로도 포함됩니다. |
|레이아웃 트리 크기|레이아웃 기록의 경우, 레이아웃 루트 아래의 총 노드 개수입니다(Chrome이 레이아웃을 시작하는 노드).|
|레이아웃 범위|가능한 값은 "Partial"(재레이아웃 경계가 DOM의 일부분인 경우) 또는 "Whole document"입니다.|
|Elements affected|Recalculate style 기록의 경우, 스타일 재계산으로 영향을 받는 요소의 개수.|
...
| 이벤트 | 설명 |
|-------|:----------|
|Composite Layers| Chrome의 렌더링 엔진으로 합성된 이미지 레이어.|
...
| 속성 | 설명 |
|-------|:----------|
|Location|페인트 이벤트의 경우, 페인트 직사각형의 x 및 y 좌표.|
|Dimensions|페인트 이벤트의 경우, 페인팅된 영역의 높이와 너비.|
