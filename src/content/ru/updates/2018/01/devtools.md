...
Note: Проверьте, какая версия Chrome вы используете на `chrome://version` . Если вы используете более раннюю версию, эти функции не будут существовать. Если вы используете более позднюю версию, эти функции могут быть изменены. Chrome автоматически обновляет новую основную версию каждые 6 недель.
...
** Локальные переопределения ** работают для большинства типов файлов с несколькими исключениями. См. [Limitations](#overrides-limitations) .
...
### Контрастность в Color Picker {: #contrast }

Теперь [Color Picker][CP] показывает коэффициент контрастности текстовых элементов. Увеличение коэффициента контрастности текстовых элементов делает ваш сайт более доступным для пользователей с нарушениями зрения и недостатками цветового зрения. См. [Color and contrast][contrast] чтобы узнать больше о том, как коэффициент контрастности влияет на доступность.
...
В ** Рисунок 5 **, две галочки рядом с ** 4.61 ** означают, что этот элемент соответствует [enhanced recommended contrast ratio (AAA)][enhanced]{:.external} . Если бы у него была только одна галочка, это означало бы, что она соответствовала [minimum recommended contrast ratio (AA)][minimum]{:.external} .
...
Нажмите ** Показать больше **! [Show More][SM]{:.cdt-inl} чтобы развернуть раздел ** Контрастность **. Белая линия в поле ** Цветной спектр ** обозначает границу между цветами, которые соответствуют рекомендуемому коэффициенту контрастности, и тем, которые этого не делают. Например, поскольку серый цвет в
...
См. [Run Lighthouse in Chrome DevTools][audit] или просмотрите A11ycast ниже, чтобы узнать, как использовать панель ** Audits ** для проверки доступности.
...
Note: Панель ** Audits ** питается от [Lighthouse][LH] . Chrome 64 запускает Lighthouse версии 2.5. Chrome 65 запускает Lighthouse версии 2.8. Таким образом, этот раздел представляет собой просто сводку обновлений Lighthouse от 2.6, 2.7 и 2.8.
...
Chrome 65 добавляет обновления в ** Step Into **! [Step Into][into]{:.cdt-inl} [Step Into][into]{:.cdt-inl} при {:.cdt-inl} в код, который передает сообщения между потоками и асинхронный код. Если вы хотите предыдущее поведение при шаге, вы можете использовать новый ** Шаг **! [Step][step]{:.cdt-inl} , вместо этого.

[into]: /web/tools/chrome-devtools/javascript/imgs/step-into.png
[step]: /web/tools/chrome-devtools/javascript/imgs/step.png

### Выполнение кода, передающего сообщения между потоками {: #workers }
...
Например, приложение в ** Рисунок 8 ** передает сообщение между основным потоком и рабочим потоком. После `postMessage()` вызов `onmessage` в основном потоке DevTools приостанавливается в обработчике `onmessage` в рабочем потоке. Обработчик undefined сам undefined сообщение обратно в основной поток. Вступая в * этот * вызов, паузы DevTools возвращаются в основной поток.
...
Например, в ** Рисунок 10 ** после `setTimeout()` в `setTimeout()` , DevTools запускает весь код, ведущий к этой точке за кулисами, а затем делает паузу в функции, переданной в undefined .
...
Панель ** Performance ** теперь позволяет временно сохранить до 5 записей. Записи удаляются при закрытии окна DevTools. См. [Get Started with Analyzing Runtime Performance][runtime] чтобы получить доступ к панели ** Performance **.
...
Вы также можете использовать Puppeteer для раскрытия функций DevTools во время просмотра без явного открытия DevTools. См. [Using DevTools Features Without Opening DevTools][without] для примера.

[without]: /web/updates/2018/01/devtools-without-devtools

## Запрос от команды DevTools: рассмотрим Canary {: #canary }

Если вы находитесь на Mac или Windows, рассмотрите возможность использования [Chrome Canary][canary] качестве браузера разработки по умолчанию. Если вы сообщаете об ошибке или изменениях, которые вам не нравятся, пока они все еще находятся в Канарее, команда DevTools может быстрее реагировать на ваши отзывы.
