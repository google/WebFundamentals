...
La détection de la prise en charge du paint Paint Worklet dans JS peut être effectuée en vérifiant l'objet `CSS`:
...
Dans les navigateurs _with_ support for paint worklet, la deuxième déclaration de `background-image` écrase la première. Dans les navigateurs _sans_ le support des travaux de peinture, la deuxième déclaration est invalide et sera supprimée, laissant la première déclaration en vigueur.
...
## Penser en dehors de la «boîte» La plupart des gens commencent à penser aux images d’arrière-plan et aux images de bordures lorsqu’ils découvrent les travaux de peinture. Un cas d'utilisation moins intuitif pour paint worklet est `mask-image` pour que les éléments DOM aient des formes arbitraires. Par exemple un [diamant](https://googlechromelabs.github.io/houdini-samples/paint-worklet/diamond-shape/):
