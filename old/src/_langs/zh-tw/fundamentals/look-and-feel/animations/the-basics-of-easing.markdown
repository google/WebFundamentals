...
introduction: "自然界中沒有樣一東西是從一個點直線移動到另一個點。 在現實中，事物在移動時總會加速或減速。 我們的大腦設計會期待這類動作，所以在動畫處理時，應該加以利用它。 自然動作會讓您的使用者更適應您的應用程式，反過來會帶來更好的整體使用者體驗。"
article:
...
{% link_sample _code/box-move-linear.html %}請參閱線性動畫。{% endlink_sample %}
...
{% link_sample _code/box-move-ease-out.html %}請參閱緩出動畫。{% endlink_sample %}
...
{% link_sample _code/box-move-ease-in.html %}請參閱段緩入動畫。{% endlink_sample %}
...
{% link_sample _code/box-move-ease-in-out.html %}請參閱段緩入緩出動畫。{% endlink_sample %}
